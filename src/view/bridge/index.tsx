...
import { getBridgePrice, getSwapBalance, getWithdrawTx } from '../../API/bridge';
...
const App: React.FC = () => { 
  ...
  const handleSubmit = async () => {  
    ...
    showLoding(true)
    try {
      const { data } = await getSwapBalance(coinList[toCoinIndex]?.id)
      if (Number(data) < Number((swapPrice * depositAmount) || 0)) {
        notification.open({
          message: t('249') + data,
          key: 'tips'
        })
    ...
    try {  
      if (chainId !== coinList[sendCoinIndex]?.id) {
        ...
        else if (coinList[sendCoinIndex]?.name === "ETH") {
          await onSwitch(isMain ? customNetwork_Base : customNetwork_Base_TEST);
        }
        else {
          notification.open({
            message: t('201'),
    ...
    let _minDepositNum: any = ''
    await Contracts.example
      ?.getSwapMinDepositAmount()
      .then((res: any) => {
        _minDepositNum = Web3.utils.fromWei(res.toString());
      })
      .catch((err: any) => {
        console.info(err);
      });
    if (Number(depositAmount) < Number(_minDepositNum)) {
      notification.open({
        message: `Latest exchange amount:${_minDepositNum}`,
        key: 'tips'
      });
      showLoding(false)
      return
    }
    console.info('_minDepositNum' + _minDepositNum) 

    Contracts.example
      ?.Deposit(web3ModalAccount, depositAmount, coinList[toCoinIndex]?.id)
      .then((res: any) => {
        pollTx(res?.transactionHash)
      ...
      .catch((error: any) => { 
        ...
        else if (typeof error?.message === 'string' && error.message.includes('Internal JSON-RPC error')) {
          try {
  ...
  let pollTimer: NodeJS.Timeout | any = null
  const handleGetHash = (txhash?: string) => {
    getWithdrawTx({
  ...
  const pollTx = (txhash: string) => {
    if (pollTimer) {
      clearInterval(pollTimer)
      pollTimer = null
    }
    pollTimer = setInterval(() => {
      handleGetHash(txhash)
    }, 3000)
  }
  useEffect(() => {
    return () => {
  ...
  const coinList = [
    ...
    {
      icon: BaseIcon, name: 'ETH',
      id: (isMain ? customNetwork_Base?.id :
        customNetwork_Base_TEST?.id),
    }
  ];
      ...
      const rpcUrls =
        coinList[sendCoinIndex]?.name === 'SEI' ?
          ...
          coinList[sendCoinIndex]?.name === 'BNB' ?
            ...
            coinList[sendCoinIndex]?.name === 'ETH' ?
  ...
  const [swapPrice, setSwapPrice] = useState<any>();
  const handleBridgePrice = async () => {
    getBridgePrice({
      srcChainId: coinList[sendCoinIndex]?.id,
      dstChainId: coinList[toCoinIndex]?.id
    }).then((res: any) => {
      setSwapPrice(res?.data)
    }).catch((err: any) => {
      console.info(err)
    })
  }
  ...
  useEffect(() => {
    handleBridgePrice()
  }, [sendCoinIndex, toCoinIndex])

  const state = store.getState();
  const handleSwitch = async () => {
    try {
      // if (coinList[toCoinIndex]?.name === "SEI") {
      //   await onSwitch(isMain ? customNetwork_SEI : customNetwork_SEI_TEST);
      //   console.log('SEI')
      //   console.log(state.chainId)
      // }
      // else if (coinList[toCoinIndex]?.name === "BNB") {
      //   await onSwitch(isMain ? customNetwork_BSC : customNetwork_BSC_TEST);
      //   console.log('BNB')
      //   console.log(state.chainId)
      // }
      // else if (coinList[toCoinIndex]?.name === "ETH") {
      //   await onSwitch(isMain ? customNetwork_Base : customNetwork_Base_TEST);
      // }
      // else {
      //   notification.open({
