...
const App: React.FC<MyComponentProps> = ({ buyNum, tabIndex }) => {
  ...
  const handleGetList = async (num: number) => {
    ...
    tokenSearch({
      ...
      offset: num,
      limit: 20,
      orderField: tabIndex === 0 ? "createdAt" : "volume",
      rangeParams: _range,
    })
      .then((res: any) => {
        if (!res.error) {
  ...
  useEffect(() => {
    ...
    // }
  }, [dataReady, pageNum, buyNum, chainId]);
  ...
  return (
    <div className="w-[386px] h5:flex-1">
      <div
        className="bg-[#1E1E1E] rounded-[20px] px-[20px] pt-[26px] pb-[20px] w-[386px]
      h5:px-[14px] h5:pt-[60px] h5:ml-15 h5:w-auto
      "
      >
